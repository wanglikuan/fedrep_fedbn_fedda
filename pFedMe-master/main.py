...
from FLAlgorithms.servers.serverattention import FedDA
from FLAlgorithms.servers.servernewperavg import newPerAvg
    ...
    for i in range(times):
        ...
        if(model == "cnn"):
            if(dataset == "Mnist"):
                ...
                model = CNNTarget().to(device), model
        ...
        if(algorithm == "FedDA"):
            server = FedDA(device, dataset, algorithm, model, batch_size, learning_rate, beta, lamda, num_glob_iters, local_epochs, optimizer, numusers, i)

        if(algorithm == "newPerAvg"):
            server = newPerAvg(device, dataset, algorithm, model, batch_size, learning_rate, beta, lamda, num_glob_iters, local_epochs, optimizer, numusers, i)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--algorithm", type=str, default="pFedMe",choices=["pFedMe", "PerAvg", "FedAvg", "FedDA","newPerAvg"]) 
